...
class OriginHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # CAZUL 1: Cache long ttl (Hit garantat in CDN la urmatoarea cerere)
        if self.path == '/static/image.jpg':
            ...
            self.send_header('Cache-Control', 'public, max-age=3600')  # Expira in 1 ora
            self.send_header('ETag', etag)
            self.end_headers()
            self.wfile.write(body.encode('utf-8'))
            print("  -> Trimitem 200 OK + Payload complet")

        # CAZUL 2: Cache short ttl cu Stale-While-Revalidate
        ...
        # CAZUL 3: Date Strict Confidentiale (WAF/CDN Bypass)
        ...
        # CAZUL 4: Must-Revalidate (Validare fortata la fiecare 5 secunde)
    ...
    def do_POST(self):
        ...
        # CAZUL 5: Mutatii (Invalidarea cache-ului in CDN)
        if self.path == '/api/data':
            # Un request POST aici trebuie sa declanseze functia invalidate_mutations() din CDN-ul tau!
